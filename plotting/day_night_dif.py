...
# Opening JSON file
f = open('../../DBs/RAN_noise_jsons_2022/day_night_ext.json')
...
city_list = {'City':['Naples','Ischia'],'Latitudes':[40.853294,40.756883],'Longitudes':[14.305573,13.877513]}
...
axs[-3].text(0.55, -0.08, 'Red = Day Noisier', transform=axs[per_idx].transAxes, size=8)
plt.savefig('../../Figures/Dif_Day_Night2/' + year2 + '/Median/daynight.png',dpi=300, bbox_inches='tight')


# Anyperiod noisy station
# Load Station Info
df = pd.read_json('../../DBs/RAN_noise_jsons_2022/day_night_ext.json')
df.columns = [P.minute*60+P.second+P.microsecond/1000000 for P in df.columns]
df1 = df.sort_index(axis=1)
nums = 0
for i, row in df1.iterrows():
	if i in completeness[completeness.Percentage >= 50].Sta.tolist():
		if (0 < row.iloc[:-10]).any():
			nums += 1
print(nums,(nums/494)*100)