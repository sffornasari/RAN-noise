...
def draw_map(i,fig,ax,stname,stlos,stlas,data,nw,df):
	...
	if i == 0:
		...
		for nw in unique_nws:
			...
			day_map = axins.scatter(df_nw.STLO, df_nw.STLA, marker=markers[nw], c=colors[nw],#df_nw.Days,
	...
	# Legend
		...
		# ni = mlines.Line2D([], [], color=colors['NI'], marker=markers['NI'], linestyle='None',
		# 						  markersize=3, label='NI')
		# rr = mlines.Line2D([], [], color=colors['RR'], marker=markers['RR'], linestyle='None',
		# 						  markersize=3, label='RR')
		# ha = mlines.Line2D([], [], color=colors['HA'], marker=markers['HA'], linestyle='None',
		# 						  markersize=3, label='HA')
	...
	# 	# plt.colorbar(day_map, cax=cbar_ax)
	# 	divider = make_axes_locatable(ax)
	# 	ax_cb = divider.new_horizontal(size="5%", pad=0.1, axes_class=plt.Axes)
	# 	fig.add_axes(ax_cb)
	# 	plt.colorbar(day_map,location='bottom', cax=ax_cb)


	# 	cbar = fig.colorbar(day_map, cax=ax,fraction=0.046, pad=0.04)
	# 	# divider = make_axes_locatable(ax)
	# 	# cax = divider.append_axes("right", size="5%", pad=0.05)
	# 	# # plt.colorbar(im, cax=cax)
	# 	# cbar = fig.colorbar(day_map, cax=cax)
...
# Save Figure
plt.savefig('../../Figures/completedays.png',dpi=300, bbox_inches='tight')