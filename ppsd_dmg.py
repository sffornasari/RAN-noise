...
#
#
# Modified DMG version
# Filename: spectral_estimation_DMG.py
#  Purpose: Various Routines Related to Spectral Estimation
#  Author: Simone Francesco Fornasari
#  Modified and custom functions are identified by a comment preceding them
#
# -----------------------------------------------------------------------------
"""
...
:copyright:
    The ObsPy DeACCopment Team (devs@obspy.org)
:license:
...
class PPSD(object):
    ...
    def __init__(self, stats, metadata, skip_on_gaps=False,
        ...
        * Providing a dictionary containing poles and zeros information. Be
        ...
        :note: When using `special_handling="ringlaser"` the applied processing
               steps are changed. Differentiation of data (converting ACCocity
        ...
        self.subchunk_fraction = subchunk_fraction
        self.subchunk_overlap_fraction = subchunk_overlap_fraction
        ...
        if self.special_handling == 'ringlaser':
            ...
            pass
        # Add special handling option for accelerometer
        ...
        #  - make 1/subchunk_fraction subchunks overlapping by subchunk_overlap_fraction 
        nfft = nfft * self.subchunk_fraction
        ...
        # original matplotlib.mlab.psd substituted in favor of scipy.signal.welch to improve performances
        #_, freq = mlab.psd(np.ones(self.len), self.nfft, self.sampling_rate, noverlap=self.nlap)
        freq, _ = welch(x=np.ones(self.len), fs=self.sampling_rate, window=self.win, nperseg=self.nfft, noverlap=self.nlap, nfft=self.nfft, return_onesided=True, scaling='density', axis=- 1, average='mean')
    ...
    def __process(self, tr):      
        """
        Processes a segment of data and save the psd informati  on.
        ...
        if np.ma.is_masked(tr.data):
            if np.sum(tr.data.mask) > 0.1 * self.ppsd_length * self.sampling_rate:
                return False
        ...
        #                       scale_by_freq=True)
        _freq, spec = welch(x=tr.data, fs=self.sampling_rate, window=self.win, nperseg=self.nfft,
                    noverlap=self.nlap, nfft=self.nfft, detrend='linear',
        ...
        if self.smoothing == True:
            ...
            self.__insert_processed_data(tr.stats.starttime, spec)
    ...
    def _get_response_from_parser(self, tr):
        ...
        resp = evalresp(t_samp=self.delta, nfft=self.nfft,
                        ...
                        units="ACC", freq=False, debug=False)
        return resp
    ...
    def plot(self, filename=None, show_coverage=True, show_histogram=True,
        ...
        :param show_earthquakes: Enable/disable plotting of earthquake models
            ...
            magnitudes 1.5 to 7.5 are available. AlternatiACCy, a distance can
    ...
    def save_json(self, jsonfile):
        with open(f'{jsonfile}.json', 'w') as outfile:
            for t, psd in zip(self.times_processed, self.psd_values):
                outfile.dump({'psds':{f'{t.hour}_{t.minute}': psd}})
